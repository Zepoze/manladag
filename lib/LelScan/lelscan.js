...
async function downloadChapter(manga_index,chap) {
    ...
    let dir
    if (!fs.existsSync(dir = `${this.downloadDir}/${mangas[manga_index].name}`)){
        fs.mkdirSync(dir)
    }

    if (!fs.existsSync(dir = `${this.downloadDir}/${mangas[manga_index].name}/${chap}`)){
        fs.mkdirSync(dir)
    }
    let i
    for (i = 0;i<nbPages;i++) {
        try {
            await downloadImage(manga_index,chap,i,this.downloadDir)
    ...
    return `${this.downloadDir}/${mangas[manga_index].name}/${chap}`
}
