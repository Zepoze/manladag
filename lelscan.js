...
const dirSource = Path.join(__dirname,'Mangas','LelScan')
let dir
if (!fs.existsSync(dir = `${dirSource}`)){
  fs.mkdirSync(dir);
}
...
async function downloadImage (manga_index,chapter,page) { 
  
  const Url = `${url+'/mangas'+mangas[manga_index].pagePath}/${chapter}/${page<10 ? '0'+page : page}.jpg`
  const path = `${dirSource}/${mangas[manga_index].name}/${chapter}/page-${page<10 ? '0'+page : page}.jpg`
...
async function downloadChapter(manga_index,chap) {
    ...
    let dir
    if (!fs.existsSync(dir = `${dirSource}/${mangas[manga_index].name}`)){
        fs.mkdirSync(dir);
    }

    if (!fs.existsSync(dir = `${dirSource}/${mangas[manga_index].name}/${chap}`)){
        fs.mkdirSync(dir);
    }
    for (let i = 0;i<nbPages;i++) {
        await downloadImage(manga_index,chap,i)
        this.emit('page-download-finished',{ page: i+1, nbPages })
    }

    return `${dirSource}/${mangas[manga_index].name}/${chap}`
}
